      ...
      {taData && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: 'Günlük Trend', value: taData.trend, icon: <TrendingUp size={14} />, bull: taData.trend.toLowerCase().includes('bull') || taData.trend.toLowerCase().includes('yükseliş') },
              { label: 'ADX Trend Gücü', value: taData.market_regime.adx.toString(), icon: <LineChart size={14} />, bull: taData.market_regime.adx >= 25 ? true : null },
              { label: 'Piyasa Rejimi', value: taData.market_regime.regime, icon: <Gauge size={14} />, bull: null },
              { label: 'Güven Seviyesi', value: taData.confidence, icon: <Shield size={14} />, bull: null },
            ].map((item) => (
              <div key={item.label}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="divide-y divide-border/15">
              {[
                { label: 'RSI (14)', value: `${taData.rsi.value.toFixed(1)} — ${taData.rsi.status}`, icon: <BarChart3 size={14} /> },
                { label: 'MACD', value: taData.macd, icon: <Activity size={14} /> },
                { label: 'Bollinger', value: taData.bollinger_status, icon: <Target size={14} /> },
                ...
                { label: 'Destek', value: `₺${taData.support_resistance.support.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`, icon: <Shield size={14} /> },
                { label: 'Direnç', value: `₺${taData.support_resistance.resistance.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`, icon: <AlertTriangle size={14} /> },
                { label: 'Stop-Loss (ATR)', value: `₺${taData.atr_stop_loss.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}`, icon: <Shield size={14} /> },
            ...
            <div className="space-y-4">
              <div className="space-y-3">
                <span className="text-base font-semibold text-foreground block">Skor Bileşenleri</span>
                {[
              ...
              <div className="space-y-2">
                <span className="text-base font-semibold text-foreground block">Uyumsuzluklar</span>
          ...
          {taData.signals.length > 0 && (
            <div>
              <span className="text-base font-semibold text-foreground block mb-2">Aktif Sinyaller</span>
