...
function CompanyDetailPage() {
  ...
  const chatContext = `sirket:${ticker.toUpperCase()}`
  ...
  useEffect(() => {
    ...
    async function fetchCompanyDetails() {
      try {
        const res = await fetch(`${baseUrl}/api/market/symbol/${ticker.toUpperCase()}`)
        if (res.ok) {
          ...
          if (data && data.success && data.data) {
            ...
            if (isMounted) {
              setCompanyStats({
                name: (companyNames as Record<string, string>)[ticker.toUpperCase()] || ticker,
                ...
                sector: "Sektör bilgisi yükleniyor..."
              })
            }
          }
        }
  ...
  if (loading || !companyStats) {
    return (
      <PublicPageLayout context={chatContext} placeholder={`${companyStats?.code || ticker} hakkında bir soru sorun...`}>
    ...
    <PublicPageLayout context={chatContext} placeholder={`${companyStats.code} hakkında bir soru sorun...`}>
      <div className="space-y-6 animate-in fade-in duration-400 flex flex-col min-h-fit pb-32">
        ...
        <div className="text-center py-12 text-muted-foreground bg-card/20 rounded-2xl border border-border/40">
          ...
          <p className="text-xs text-muted-foreground/60">Ticker: {companyStats.code}</p>
        </div>
