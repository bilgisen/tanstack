...
import { KAPFeedSkeleton, NotificationCard } from '../kap/NotificationCard'

export function SektorBildirimlerList({ sectorName }: { sectorName: string }) {
    ...
    <div className="space-y-5 animate-in fade-in duration-400 max-w-5xl mx-auto py-2">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h2 className="text-base font-bold text-foreground uppercase tracking-wider">
            {sectorName} Bildirimleri
          </h2>
          <p className="text-xs text-muted-foreground">
            Sektör şirketlerinin KAP bildirimleri
            {data && !isLoading && (
      ...
      {!isLoading && !isError && notifications.length === 0 && (
        <div className="rounded-2xl border border-border/60 p-10 text-center text-sm text-muted-foreground">
          Bu sektör için kayıtlı KAP bildirimi bulunamadı.
        </div>
      )}
